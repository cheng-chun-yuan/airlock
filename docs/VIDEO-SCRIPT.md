# Demo video script (narration)

Record your own voice over the cut demo (2:31). ETHGlobal rules: 2–4 minutes, at least 720p, **your own narration**: no text-to-speech, no AI voiceover.

Total with intro and outro ≈ 2:50. Timecodes are positions in the cut video. Speak at a relaxed pace; the lines are sized to fit.

---

**[Intro · title card, before the video · ~12 s]**
> This is Airlock. AI agents leak confidential data the moment they call a frontier model. Airlock is the airlock between them: local AI by default, frontier AI only with human consent, and accountability on-chain.

**[0:00 · 01 Public]**
> An agent talks to Airlock like any OpenAI endpoint. A public question has nothing sensitive in it, so it goes straight out, and it's still audited.

**[0:12 · 02 Confidential]**
> Now a confidential contract review. Airlock de-identifies names, money and email into labels. I can read them here, but the model only ever sees ORG-1 and PERSON-1. A local model already tried to re-identify them and failed, so risk is low. The policy comes from ENS: legal has to approve. I hold, and verify with World ID for Agents. Approved. Only the labels left the building, and the answer streams back with the real names restored.

**[0:40 · 03 Seal]**
> Same request, but this time I seal it, with a note. Nothing leaves. The local model answers instead, and the employee sees why.

**[0:53 · 04 Revoked]**
> Carol is an approver. I revoke her ENS subname on Sepolia. It's one transaction, and MultiBaas pushes the event to the gateway right away. She's still a verified human, but her role is gone, so nothing is sent.

**[1:14 · 05 Two humans]**
> Here the name TSMC is hidden, but the context gives it away, and our re-identification test catches it. High risk means the ENS policy requires two different humans. Alice approves: one of two. Now "Bob" tries with the same World ID, and it's refused. World ID knows it's the same person, without knowing who. An account system can't do that without KYC. A real second human approves, and it goes out.

**[1:55 · 06 New agent]**
> A brand-new agent that nobody registered gets the org's default policy through ENSv2 wildcard resolution. Confidential data is blocked, with zero setup.

**[2:08 · 07 Proof]**
> Everything is on the record. Agents link to one shared policy record, so you edit it once and all of them change. Only the security key can change policy; the gateway can only write approvers and the audit root. MultiBaas shows every on-chain change, and every decision is hash-chained and anchored to ENS.

**[Outro · after 2:31 · ~5 s]**
> Airlock. Local AI by default, frontier AI by human consent, accountability on-chain.

---

## Recording tips
- Put the title card (project name + one line) before the video, and a closing card with the GitHub link after it.
- Read slightly slower than feels natural; the cut leaves short pauses after each result, and that's where the key line should land.
- If a line runs long, drop the second sentence rather than rushing. The on-screen result carries it.
